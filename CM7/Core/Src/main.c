...
#define RPMSG_CHAN_NAME              "openamp_rtos"
...
char str2cm4[] = "Bare-Metal programming - sending string from CM7 to CM4 core using OpenAMP framework";
...
/*
 * The new_service_cb() function is a callback that gets triggered when a new RPMsg service or endpoint becomes
 * available for communication. It allows the local processor to take action when a remote core
 * (such as the Cortex-M4 or Cortex-M7 in an STM32H7) advertises a new service that the local
 * processor can communicate with.
 *
...
void StartRxTask(void *argument)
{
  ...
  /*
   * This function in the OpenAMP framework is used to wait for a specific RPMsg endpoint
   * to become ready before sending or receiving messages. This function ensures that the
   * communication channel (or endpoint) between two cores or processors is properly
