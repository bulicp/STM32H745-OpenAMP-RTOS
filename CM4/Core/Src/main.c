...
#define RPMSG_SERVICE_NAME              "openamp_rtos"
...
void StartRxTask(void *argument)
{
  ...
  */
  ...
  // Acquire semaphore so that Tx task does not run:
  if (osSemaphoreAcquire(txSemHandle, osWaitForever) != osOK) Error_Handler();
